...
class Tello(object):
    ...
    def send_command(self, command):
        """ Sending Command 
            ref: https://github.com/Virodroid/easyTello/blob/master/easytello/tello.py
        """
        try:
            self.log_easytello.append(Stats(command, len(self.log_easytello)))
            self.sock.sendto(command.encode('utf-8'), self.tello_addr)
            print("sendding command")

            start = time.time()
            while not self.log_easytello[-1].got_response():  # Blocking operation while no response in the log
                now = time.time()
                if now - start > 5.0: # longer than 5.0 seconds
